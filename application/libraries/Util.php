...
class Util {
    ...
    public static function getEncryptParams($params){
      return base64_encode((is_array($params) ? json_encode($params) : $params));
    }
    
    public static function getDecryptParams($params){
      $json = base64_decode($params);
      return (Array) json_decode($json);
    }
    
    public static function paramsToUrlGet($params, $before = '?'){
      $args = array();
      foreach($params as $key => $val){
	$args[] = $key.'='.$val;
      }
      return $before.implode('&', $args);
    }
    
    public static function getDecryptParamsUrl($params){
      return self::paramsToUrlGet(self::getDecryptParams($params));
    }
