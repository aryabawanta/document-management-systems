...
class Workunit_model extends AppModel {
    protected $_table = 'workunits';

    public function __construct() {
        parent::__construct();
        $this->_table = 'workunits';
    }
    ...
    public function getStructures($parent=0, $use_json=true){
        $sql = "select id, name as text from workunits where parent=$parent and is_delete=0 order by name";
        $rows = dbGetRows($sql);
        if (!empty($rows)){
