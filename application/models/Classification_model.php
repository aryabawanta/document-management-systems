...
class Classification_model extends AppModel {
    protected $_table = 'classifications';

    public function __construct() {
        parent::__construct();
        $this->_table = 'classifications';
    }
    ...
    public function getStructures($parent=0, $use_json=true){
        $sql = "select id, name as text, code from $this->_table where parent=$parent and is_delete=0 order by code";
        $rows = dbGetRows($sql);
        if (!empty($rows)){
