...
class Classification extends WEB_Controller {

    protected $title = 'Classifications';
    protected $class_name = "Classifications";
    ...
    public function index() {
        ...
        $this->data['method_name'] = $this->data['title'] = "List of ".$this->class_name;

        $buttons = array();
        $buttons['add'] = array('label' => ' Add', 'type' => 'success', 'icon' => 'plus', 'click' => 'goAdd()');
        $buttons['import'] = array('label' => ' Import', 'type' => 'info', 'icon' => 'file-send', 'click' => 'goImport()');
    ...
    public function edit($id = null) {
        ...
        $arr_locations = $this->model->show_sql(false)->filter("where is_delete=0")->getAll();
        $locations = array();
        $locations[0] = "Tidak ada - Merupakan Klasifikasi Tertinggi";
        foreach ($arr_locations as $location) {
            $locations[$location['id']] = $location['code'].' - '.$location['name'];
        }
        // get data
        if (isset($id)){
    ...
    public function update($id = null) {
        ...
        $record_log['description'] = "Successfully Change ".$this->class_name." '".$data['name']."'";
        ...
        if ($update === true) {
            $ok = true;
            $msg = "Successfully Change ".$this->class_name." ".$data['name'];
            ...
            if (!is_string($update)) {
                ...
                if (empty($validation))
                    $msg = "Failed to Change ".$this->class_name." ".$data['name'];
                else
    ...
    public function create() {
        ...
        $record_log['description'] = "Add ".$this->class_name." '".$data['name']."'";

        $insert = $this->model->create($data, TRUE, TRUE);
        if ($insert) {
            $ok = true;
            $msg = "Successfully Add ".$this->class_name;
            SessionManagerWeb::setFlashMsg($ok, $msg);
            ...
            if (!is_string($insert)) {
                $validation = $this->model->getErrorValidate();
                if (empty($validation))
                    $msg = "Failed to Add ".$this->class_name;
                else
                    $msg = implode('<br />', $validation);
            } else
                $msg = $insert;            
